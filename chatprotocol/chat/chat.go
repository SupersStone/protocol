...
const charset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_" // Includes Chinese characters

func GenerateRandomString(length int) (string, error) {
	result := make([]rune, length)
	charsetLen := big.NewInt(int64(utf8.RuneCountInString(charset))) // Handle UTF-8 correctly
	...
	for i := 0; i < length; i++ {
		...
		result[i] = rune(charset[randomIndex.Int64()])
	}
...
func VerifyAccout(accout string) bool {
	regexStr := fmt.Sprintf("^[%s]+$", charset) // 匹配charset中字符组成的字符串
	regex := regexp.MustCompile(regexStr)

	if regex.MatchString(accout) {
		return true
	}
...
func UrlToBase64QRCode(url string, size int) (string, error) {
	...
	// 使用bytes.Buffer来避免多次内存分配
	buf := new(bytes.Buffer)
	err = png.Encode(buf, qrCode.Image(size))
	if err != nil {
		return "", fmt.Errorf("编码PNG图片失败: %w", err)
	}
